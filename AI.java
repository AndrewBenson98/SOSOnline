...
public class AI {
	...
	// /**
	// * Simpler AI that doesnt do the S trap
	// *
	// * @param grid
	// * @return
	// */
	// public boolean dumbAI(char[][] grid) {
	// if (FindWin(grid)) {
	// points++;
	// System.out
	// .println("POINT
	...
	public void totallyRandom(char[][] grid) {
		...
		if (Index != 0) {
			if (carefulSPlacement(grid, coordinates[1][randIndex], coordinates[0][randIndex])) {
				...
				// System.out.println("careful random S");
				return;
				...
				// System.out.println("careful random O");
			...
			for (int x = boarder; x < grid.length - boarder; x++) {
				if (tempGrid[y][x] == '-') {
					...
					while (FindWin(tempGrid)) {
						count++;
						// System.out.println("WIN FOUND S");
					}
					...
					if (count < lowestCombo) {
						lowestCombo = count;
						// System.out.println("Lowest Combo S : "+lowestCombo);
						bestChar = 'S';
						bestX = x;
						bestY = y;
					}
					...
					while (FindWin(tempGrid)) {
						count++;
						// System.out.println("WIN FOUND O ");
					}

					if (count < lowestCombo) {
						lowestCombo = count;
						// System.out.println("Lowest Combo O : "+lowestCombo);
						bestChar = 'O';
						bestX = x;
						bestY = y;
					}
